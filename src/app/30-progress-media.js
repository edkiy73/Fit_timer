...
function openPhotoFull(i){
  ...
  renderPhotoFull();
  fitUi.openModal($('photoFullModal'));
}
...
$('photoFullModal').onclick = e => { if(e.target === $('photoFullModal')) fitUi.closeModal($('photoFullModal')); };
async function delCmpPhoto(which){
  ...
  await savePhotos();
  renderPhotos();
  if(photos.length < 2){ fitUi.closeModal($('cmpModal')); return; }
  fillCmpSel($('cmpA'), 0);
