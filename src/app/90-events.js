...
const notificationPreferenceStore = fitNotifications.createPreferenceStore({
...
async function refreshVoicePackUI(progressEvent){
  ...
  for(const row of [
    ...
    s.textContent=label;
    fitUi.setBusy(b, running, {busyText:button, idleText:button, disabled});
...
async function downloadSelectedVoicePack(){
  ...
  for(const id of ['btnVoicePack','btnHfVoicePack']){
    fitUi.setBusy($(id), true, {busyText:t('voicepack.downloadingBtn')});
  }
