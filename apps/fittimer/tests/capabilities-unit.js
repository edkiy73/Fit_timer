...
const capsModule = require('../../../packages/core/server/capabilities-core');
...
ok('client and server capability lists match',
  CAPABILITY_NAMES.every(name => fs.readFileSync('../../packages/core/src/core/capabilities.ts','utf8').includes(`'${name}'`))
  && fs.readFileSync('../../packages/core/src/types/core.ts','utf8').includes('voice: boolean'));
...
const capSource = fs.readFileSync('../../packages/core/server/capabilities-core.js','utf8');
...
async function withCapabilities(flags, fn){
  ...
  const target = path.resolve('../../packages/core/server/ai-endpoint.js');
...
(async () => {
  ...
  const authSource = fs.readFileSync('../../packages/core/server/auth-core.js','utf8');
  ...
  ok('mobile composition wires native integrations through capabilities',
    /from ['"]@appbase\/core\/capabilities\.js['"]/.test(mobile)
