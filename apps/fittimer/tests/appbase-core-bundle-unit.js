...
const app=fs.readdirSync(APP_DIR).filter(name=>name.endsWith('.js')).map(name=>fs.readFileSync(APP_DIR+name,'utf8')).join('\n');
const bridge=fs.readFileSync('scripts/test-bridge.mjs','utf8');
const esm=fs.readFileSync('scripts/build-esm.mjs','utf8');
...
ok('product runtime is ES modules, not a concatenated app.js',
  !build.includes("target: 'app.js'") && !fs.existsSync('app.js') && fs.existsSync('src/app/index.js'));
...
ok('ESM entry starts the native bridge before the product runtime chunk',
  ...
  && /await import\('\.\/app\/index\.js'\)/.test(mainSource)
