...
Paths are relative to `apps/fittimer/` (FitTimer inside the monorepo). AppBase Core lives in `../../packages/core/`.
...
| `../../packages/core/src/core/storage.ts` | generic local KV storage | storage/Core work |
| `../../packages/core/src/core/identity.ts` | generic Account/Profile defaults | account/profile Core boundary |
| `../../packages/core/src/core/sync.ts` | generic client document registry/scopes | sync/Core work |
| `../../packages/core/src/core/observability.ts` | generic client analytics/diagnostic transport | analytics/diagnostics Core work |
| `../../packages/core/src/core/notifications.ts` | generic notification preferences + delivery budget policy | notification/Core work |
| `../../packages/core/src/core/native-notifications.ts` | generic Capacitor local/push notification transport | mobile notification/Core work |
| `../../packages/core/src/core/mobile.ts` | generic lifecycle/URL/share/haptic/theme/biometry bridge | mobile/Core work |
| `../../packages/core/src/core/speech.ts` | generic native TTS / speech recognition / offline model transport | voice/Core work |
| `../../packages/core/src/core/capabilities.ts` + `../../packages/core/server/capabilities-core.js` | product capability switches from `config/product.json` | enabling/disabling platform capabilities |
| `lib/fit-account-extension.js` | FitTimer hooks for generic auth (trainer page/key, shared links) | auth/trainer/account deletion |
| `../../packages/core/server/product-core.js` | product identity for generic server code | brand/name in server messages |
| `../../packages/core/server/auth-core.js` / `../../packages/core/server/sync-core.js` | generic account/auth and document-sync handlers (factories) | auth/sync server logic; `api/auth.js`, `api/sync.js` only compose them |
| `../../packages/core/` (`npm run check` there) | AppBase Core package: typecheck, boundaries, runtime + template smoke tests; `template/` = neutral app composition | Core work / new app |
| `lib/product.js` | registers FitTimer `config/product.json` with server Core; first require of every `api/*` entry | server bootstrap |
| `scripts/build-esm.mjs` | esbuild bundle of `src/main.ts` + `mobile.js` with Core | client build |
| `../../packages/core/server/sync-registry.js` | generic server document registry/scopes | server sync/Core work |
| `lib/fit-sync-schema.js` + `src/app/sync-schema.ts` | FitTimer document registration | product sync semantics |
| `../../packages/core/src/core/*.ts` | canonical reusable AppBase Core implementations | Core extraction/refactoring |
...
- `src/app/10-data-sync.js` — FitTimer storage adapter, users, sync/calendar foundations; low-level KV ownership is in `../../packages/core/src/core/storage.ts`, document admission is delegated to AppBase Sync registry.
...
- `../../packages/core/server/store.js` — Redis/storage abstraction and persistence helpers.
- `../../packages/core/server/util.js` — shared server utilities.
- `../../packages/core/server/mail.js` — Resend/email.
- `lib/seed.js` — seed/catalog data.
- `../../packages/core/server/ai.js` — AI provider/runtime logic.
- `../../packages/core/server/ai-endpoint.js` — AI endpoint orchestration.
- `../../packages/core/server/analytics-core.js` — generic analytics storage/aggregation engine.
...
- `../../packages/core/server/diagnostics.js` — generic redacted client error aggregation.
