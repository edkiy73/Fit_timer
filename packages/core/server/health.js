...
async function collectHealth({probes: productProbes = []} = {}){
  ...
  if(info.connected || info.memory){
    ...
    for(const probe of productProbes) probes.push(await timed(probe.name, probe.run));
    ...
    probes.push(
      ...
      ...productProbes.map(probe => ({name:probe.name,ok:false,latencyMs:0,error:'storage_unavailable'})),
    ...
    services:{
      ...
      supabase:{
        ...
        shadow:{
          ...
          lastWrite:shadow.lastWrite||null,
          parity:shadow.parity||null,
          counters:shadow.counters||{},
          readiness:shadow.readiness||null
        }
      },
