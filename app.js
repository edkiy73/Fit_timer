...
const HF_HINTS = {
  ...
  voice: 'Скажи «дальше», «пауза» или «продолжить» — Fit Timer поймёт команду во время тренировки. Для распознавания на некоторых телефонах может понадобиться интернет.',
...
document.querySelectorAll('#hfSeg button').forEach(b => {
  b.onclick = async ()=>{
    ...
    if(mode === 'voice' && (await kvGet('voiceHint')) !== '1'){
      kvSet('voiceHint', '1');
      appAlert('Голосовое управление включено. Скажи «дальше», «готово» или «пропустить», чтобы перейти к следующему этапу. «Пауза» остановит тренировку, «продолжить» — запустит снова.\n\nНа некоторых телефонах Android может быть слышен короткий системный сигнал, когда микрофон начинает слушать.');
    }
  };
});
