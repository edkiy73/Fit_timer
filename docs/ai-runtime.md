...
5. Для `video.parse` действует отдельный grounded pipeline: сервер сначала получает таймкодированные субтитры YouTube, затем извлекает только факты с дословным evidence, а финальный ответ проверяет на неизменность порядка, повторов/времени, подходов, отдыха и таймкодов. Если субтитров нет или их недостаточно, Gemini может посмотреть публичный YouTube URL через официальный video input; видео, не являющееся тренировкой, отклоняется без генерации программы.
