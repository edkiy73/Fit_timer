...
- Обычные личные программы и программы, которыми тренер делится напрямую, остаются одноязычными и не переводятся автоматически.
- Заявка тренера в каталог тоже приходит только на исходном языке. RU + EN готовятся уже в админке: AI запускается только по кнопке модератора, а при недоступном AI перевод можно заполнить руками или вставить JSON из внешнего переводчика.
- Approve каталога запрещён, пока RU и EN не заполнены и их машинная структура/нагрузка не совпадает. При добавлении из каталога пользователю сохраняется одноязычная копия на текущем языке профиля с `p.locale`; последующие AI-правки используют этот язык.
