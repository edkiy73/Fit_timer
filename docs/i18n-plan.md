...
- Выбор языка хранится **в профиле** как `u.locale: system | <locale>`; по умолчанию — **как в системе**. Разные профили на одном устройстве могут иметь разные языки.
- `appLocale` — вычисленный эффективный язык активного профиля для интерфейса, TTS и API. Аккаунт хранит только эффективный locale для писем/серверных ответов.
...
- язык переключается без перезапуска, хранится отдельно у каждого профиля и восстанавливается после входа;
