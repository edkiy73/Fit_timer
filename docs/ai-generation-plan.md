# Fit Timer: развитие платной AI-генерации

Статус: серверная генерация, server/client validation и provider fallback уже реализованы.
Фактическая схема и точки настройки описаны в `docs/ai-runtime.md`. Этот файл — только roadmap
оставшихся улучшений: structured output/JSON, кэш, фоновые jobs, бюджетные cap, circuit breaker
и библиотека канонических картинок.
...
единый адаптер, а активные поставщики и модели выбираются в админке отдельно для
текста и изображений. У каждой операции есть основной и резервный маршрут. Тогда
провайдера можно сменить без обновления APK и без остановки Premium-функций.

Текущий fallback уже срабатывает и на provider/transport failure, и на структурно невалидный
HTTP 200. Отдельная дешёвая repair-попытка у основного провайдера пока не реализована; это
возможное будущее улучшение. Ошибки доступа/валидации входного запроса на резерв не отправлять. После
нескольких сбоев circuit breaker временно исключает провайдера, а админка
показывает причину, время последней ошибки и расход каждого маршрута.
...
4. Автоматически проверить формат, отсутствие текста/логотипов и базовые
   дефекты. Первым результат принимает или перегенерирует тот, кто его заказал.
5. Если тренер предлагает упражнение или программу в общий каталог, изображение
   вместе с материалом попадает в отдельную очередь администратору. До одобрения
   оно доступно только автору и не используется как общий эталон.
6. После принятия для каталога сжать в WebP/AVIF, положить в объектное хранилище
   и повторно использовать для всех пользователей.
...
набора. Принято решение хранить обезличенные запросы ограниченное время для
контроля качества; рекомендуемый стартовый срок — 30 дней с настройкой в админке
и немедленным удалением вместе с аккаунтом. Это нужно зафиксировать в Privacy
Policy до запуска.

## Что настраивается в админке

Админка становится центром управления платными возможностями без нового релиза
приложения:

- основной и резервный провайдеры отдельно для текста и изображений;
- конкретные модели, timeout, число повторов и состояние circuit breaker;
- Premium-лимиты, месячный hard cap, стоимость операций и аварийные выключатели;
- очередь AI-материалов, которые тренеры предложили в общий каталог;
- срок хранения обезличенных запросов и ручное удаление;
- состояние платёжных интеграций, продукты/тарифы, webhook и сверка подписок.

Секреты провайдеров и платёжки не возвращаются браузеру админки. Они хранятся
зашифрованно на сервере или в secret manager; интерфейс показывает только
состояние подключения и последние четыре символа, а новое значение разрешает
только записать или заменить. Все изменения настроек попадают в audit log.
...
3. Реализовать общий адаптер, основной и резервный текстовые маршруты и тестовый
   набор.
...
6. Добавить в админку провайдеров, лимиты, бюджет, диагностику и audit log.
7. Добавить очередь изображений, хранилище, личное принятие результата и
   админскую модерацию публикаций в каталоге.
8. Подключить управление тарифами и платёжными интеграциями после выбора схемы
   оплаты для Web, Google Play и App Store.
9. Провести закрытый запуск, измерить стоимость и только затем расширить лимиты.

## Что потребуется от владельца перед реализацией

Принятые решения:

1. Провайдеры, fallback, лимиты и платёжные подключения управляются через
   админку.
2. Сгенерированный материал сначала проверяет его автор. Публикацию тренера в
   общем каталоге дополнительно подтверждает администратор.
3. Обезличенные запросы разрешено временно хранить для улучшения качества.
4. Провайдер должен заменяться без релиза приложения; для каждого типа генерации
   обязательно задаётся резервный.

Перед реализацией от владельца останется выбрать цену Premium, допустимую
себестоимость AI на активного пользователя, окончательный срок хранения
обезличенных запросов и предоставить аккаунты выбранных провайдеров. Ключи будут
добавляться через защищённую серверную настройку, а не в APK.
...
- Не переносить существующий Gemini-ключ на сервер. Подключение основного и
  резервного провайдера делается через защищённую серверную настройку в админке.
