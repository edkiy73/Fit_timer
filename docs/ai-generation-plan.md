...
Статус: базовая серверная генерация реализована. Фактическая схема и точки
настройки описаны в `docs/ai-runtime.md`; ниже остаются следующие улучшения:
строгий JSON, кэш, фоновые jobs, бюджетные cap и библиотека картинок.
...
Fallback срабатывает при timeout, сетевой ошибке, 429 и 5xx. При ошибке формата
сначала выполняется одна дешёвая попытка исправления у основного провайдера.
Ошибка безопасности или некорректный запрос на резерв не отправляются. После
