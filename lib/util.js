...
/* ---- пределы полей на сервере ----
   ...
   приходит не только из приложения. Любой может послать в /api/trainer строку в
   мегабайт или «ссылку» javascript:, и отдавать её потом ЧУЖИМ людям на странице
   тренера мы не будем.
...
function cors(req, res){
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Fit-Email, X-Fit-Device, X-Fit-Token');
