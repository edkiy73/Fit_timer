...
const DEFAULTS = {
  ...
  update: {android:{latestCode:0, minimumCode:0, latestName:'', url:'', messageRu:'', messageEn:''}}
};
...
function sanitizeSettings(src){
  ...
  const updateUrl = line(androidUpdate.url, 500);
  const latestCode = Math.round(num(androidUpdate.latestCode, 0, 0, 999999999));
  const minimumCodeRaw = Math.round(num(androidUpdate.minimumCode, 0, 0, 999999999));
  const minimumCode = latestCode ? Math.min(minimumCodeRaw, latestCode) : 0;
  return {
    ...
    update: {
      android: {
        latestCode,
        minimumCode,
        latestName: line(androidUpdate.latestName, 40),
        url: /^(https?:\/\/|market:\/\/)/i.test(updateUrl) ? updateUrl : '',
        messageRu: line(androidUpdate.messageRu, 240),
        messageEn: line(androidUpdate.messageEn, 240)
      }
    }
  };
}
