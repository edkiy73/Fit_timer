...
const DEFAULTS = {
  ...
  update: {android:{
    ...
    store:{latestCode:0, minimumCode:0, latestName:'', url:'', messageRu:'', messageEn:''}
  }}
};
...
const endpoint = (v, allowed) => {
  ...
  return {
    provider: providers.includes(v && v.provider) ? v.provider : providers[0],
    model: line(v && v.model, 100)
  };
};
const sanitizeAndroidRelease = (src, allowMarket) => {
  src = src && typeof src === 'object' ? src : {};
  const url = line(src.url, 500);
  const latestCode = Math.round(num(src.latestCode, 0, 0, 999999999));
  const minimumRaw = Math.round(num(src.minimumCode, 0, 0, 999999999));
  return {
    latestCode,
    minimumCode: latestCode ? Math.min(minimumRaw, latestCode) : 0,
    latestName: line(src.latestName, 40),
    url: (allowMarket ? /^(https?:\/\/|market:\/\/)/i : /^https:\/\//i).test(url) ? url : '',
    messageRu: line(src.messageRu, 240),
    messageEn: line(src.messageEn, 240)
  };
};
...
function sanitizeSettings(src){
  ...
  const legacyStore = /^(market:\/\/|https?:\/\/(?:play\.google\.com|(?:www\.)?rustore\.ru))/i.test(legacyUrl);
  const directInput = androidUpdate.direct && typeof androidUpdate.direct === 'object'
    ? androidUpdate.direct : (legacyStore ? {} : androidUpdate);
  const storeInput = androidUpdate.store && typeof androidUpdate.store === 'object'
    ? androidUpdate.store : (legacyStore ? androidUpdate : {});
  const directUpdate = sanitizeAndroidRelease(directInput, false);
  const storeUpdate = sanitizeAndroidRelease(storeInput, true);
  return {
    enabled: src.enabled !== false,
    retentionDays: Math.round(num(src.retentionDays, 30, 1, 30)),
    text: {
      primary: endpoint(text.primary || DEFAULTS.text.primary, ['gemini','openai','openrouter']),
      backup: endpoint(text.backup || DEFAULTS.text.backup, ['gemini','openai','openrouter'])
    },
    image: {
      primary: endpoint(image.primary || DEFAULTS.image.primary, ['gemini','openai']),
      backup: endpoint(image.backup || DEFAULTS.image.backup, ['gemini','openai']),
    ...
    update: {
      android: {
        // Flat fields remain the direct-channel alias for APKs released before
        // channel-aware config existed.
        ...directUpdate,
        direct: directUpdate,
        store: storeUpdate
      }
...
// Текстовый ответ по длинной программе (правка 20+ упражнений) не укладывался
// в 35 секунд. Функция на Vercel живёт 60 секунд (vercel.json), поэтому основной
// провайдер получает почти всё это окно, а резерв пробуем, только если на него
// ещё остаётся время (см. generate).
const TEXT_TIMEOUT_MS = Math.max(10, +process.env.AI_TEXT_TIMEOUT_MS || 48000); // env — только для тестов
const FALLBACK_MIN_LEFT_MS = 20000;
...
async function fetchTimed(url, opts, timeoutMs){
  ...
  try{ return await fetch(url, Object.assign({}, opts, {signal:ctl.signal})); }
    ...
    // своё понятное имя вместо «This operation was aborted»: по нему приложение
    // показывает человеку «ИИ не успел ответить», а не английский текст ошибки
    if(ctl.signal.aborted) throw Object.assign(new Error('ai_timeout'), {status:504, code:'ai_timeout'});
    throw e;
  }
...
async function geminiVideoText(ep, videoUrl, prompt){
  const res = await fetchTimed(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(ep.model)}:generateContent`, {
    method:'POST', headers:{'Content-Type':'application/json','X-goog-api-key':keyFor('gemini') || ''},
    body:JSON.stringify({contents:[{parts:[
      {file_data:{file_uri:String(videoUrl || '')}},
      {text:String(prompt || '')}
    ]}], generationConfig:{maxOutputTokens:16384,temperature:.1}})
  }, 42000);
  if(!res.ok) return jsonError(res);
  const j = await res.json();
  const cand = (j.candidates || [])[0] || {};
...
async function callOne(type, ep, prompt, imageSize, aspectRatio){
  ...
  if(process.env.AI_TEST_MODE === '1'){
    ...
    if(String(prompt||'').startsWith('Translate the user-visible fitness text')){
    ...
    if(String(prompt||'').includes('=== ADMIN CATALOG REQUEST ===')){
      return {text:[
  ...
  if(type === 'image'){
    if(ep.provider === 'gemini') return geminiImage(ep, prompt, imageSize, aspectRatio);
  ...
  throw Object.assign(new Error('provider_not_supported'), {status:400});
}
...
async function generate(type, settings, prompt, options){
  ...
  const checked = async (ep, fallback) => {
    ...
    if(validate){
      ...
      if(!verdict || verdict.ok !== true){
        const err = Object.assign(new Error((verdict && verdict.reason) || 'malformed_response'), {status:502});
        err.validation = verdict || null;
        throw err;
      }
      if(verdict.text && out.text) out.text = verdict.text;
    }
    return Object.assign(out, {provider:ep.provider,model:ep.model,fallback});
  };

  let first;
  const started = Date.now();
  try{
    return await checked(route.primary, false);
  }catch(e){ first = e; }
  ...
  // основной провайдер съел почти всё время функции — резерв всё равно оборвала
  // бы платформа, и человек получил бы не понятную ошибку, а обрыв соединения
  if(Date.now() - started > FUNCTION_BUDGET_MS - FALLBACK_MIN_LEFT_MS) throw first;
  ...
  try{
    return await checked(route.backup, true);
  }catch(second){
    second.primaryError = first;
    throw second;
  }
}

async function generateGeminiVideo(settings, videoUrl, prompt, options){
  const routes = [settings && settings.text && settings.text.primary, settings && settings.text && settings.text.backup]
    .filter(Boolean);
  const validate = options && typeof options.validate === 'function' ? options.validate : null;
  const errors = [];
  for(let i=0;i<routes.length;i++){
    const ep = routes[i];
    if(ep.provider !== 'gemini' || !keyFor('gemini')) continue;
    try{
      const out = process.env.AI_TEST_MODE === '1'
        ? {text:JSON.stringify({isWorkout:false,confidence:.99,reason:'test mode',exercises:[]})}
        : await geminiVideoText(ep, videoUrl, prompt);
      if(validate){
        const verdict = validate(out);
        if(!verdict || verdict.ok !== true){
          const err = Object.assign(new Error((verdict && verdict.reason) || 'malformed_response'), {status:502});
      ...
      return Object.assign(out,{provider:'gemini',model:ep.model,fallback:i>0});
    }catch(e){
      errors.push(e);
      if(!mayFallback(e)) throw e;
    }
  }
  const err = errors[0] || Object.assign(new Error('gemini_video_unavailable'), {status:503});
  throw err;
}
...
module.exports = { DEFAULTS, DEFAULT_PRICES, sanitizeSettings, getSettings, providerStatus, billingProviderStatus, generate, generateGeminiVideo };
