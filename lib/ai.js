...
async function fetchTimed(url, opts, timeoutMs){
  ...
  // Две попытки должны целиком уместиться в 60 секунд Vercel: основной маршрут
  // получает 27 секунд, остаток остаётся резервному и сериализации ответа.
  const timer = setTimeout(()=> ctl.abort(), timeoutMs || 27000);
