...
function sanitizeSettings(src){
  ...
  return {
    ...
    prices,
    payment: {
      provider: line(payment.provider || 'not_connected', 40),
      androidMonth: line(payment.androidMonth, 120), androidYear: line(payment.androidYear, 120),
...
async function fetchTimed(url, opts, timeoutMs){
  ...
  // Проект пока на 10-секундном лимите Vercel. Быстрый ответ или быстрый отказ
  // успевают перейти на резерв; длинные картинки потребуют фоновой очереди.
  const timer = setTimeout(()=> ctl.abort(), timeoutMs || 8500);
...
async function geminiText(ep, prompt){
  const res = await fetchTimed(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(ep.model)}:generateContent`, {
    ...
    body:JSON.stringify({contents:[{parts:[{text:prompt}]}], generationConfig:{maxOutputTokens:32768,temperature:1}})
  });
...
async function geminiImage(ep, prompt){
  const res = await fetchTimed(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(ep.model)}:generateContent`, {
    method:'POST', headers:{'Content-Type':'application/json','X-goog-api-key':keyFor('gemini') || ''},
...
async function callOne(type, ep, prompt){
  ...
  if(type === 'image') return ep.provider === 'gemini' ? geminiImage(ep, prompt) : openaiImage(ep, prompt);
...
async function generate(type, settings, prompt){
  const route = settings[type];
  let first;
  try{
    const out = await callOne(type, route.primary, prompt);
  ...
  const out = await callOne(type, route.backup, prompt);
  return Object.assign(out, {provider:route.backup.provider,model:route.backup.model,fallback:true});
}

module.exports = { DEFAULTS, DEFAULT_PRICES, sanitizeSettings, getSettings, providerStatus, generate };
