...
async function geminiImage(ep, prompt, imageSize){
  const size = ['512','1K','2K','4K'].includes(String(imageSize || '')) ? String(imageSize) : '1K';
  const res = await fetchTimed(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(ep.model)}:generateContent`, {
    method:'POST', headers:{'Content-Type':'application/json','X-goog-api-key':keyFor('gemini') || ''},
    body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{responseModalities:['TEXT','IMAGE'],imageConfig:{imageSize:size}}})
  });
  if(!res.ok) return jsonError(res);
  const j = await res.json();
...
async function callOne(type, ep, prompt, imageSize){
  ...
  if(type === 'image') return ep.provider === 'gemini' ? geminiImage(ep, prompt, imageSize) : openaiImage(ep, prompt);
...
async function generate(type, settings, prompt){
  ...
  let first;
  try{
    const out = await callOne(type, route.primary, prompt, type === 'image' ? route.size : null);
  ...
  const out = await callOne(type, route.backup, prompt, type === 'image' ? route.size : null);
