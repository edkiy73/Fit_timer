...
async function sendAndroid(token,message){
  ...
  if(!res.ok)throw Object.assign(new Error('fcm_failed'),{status:res.status,detail:await res.text().catch(()=> '')});
}
let apnsJwt=null;
function apnsToken(){
...
function sendIos(token,message){return new Promise((resolve,reject)=>{
  ...
  req.on('response',h=>{status=+h[':status']||0;});req.setEncoding('utf8');req.on('data',c=>body+=c);req.on('end',()=>status>=200&&status<300?finish(null,{ok:true}):finish(Object.assign(new Error('apns_failed'),{status,detail:body})));
...
async function sendPushToAccountHash(mh,message){
  ...
  const ds=Object.values(acc.pushDevices||{}).filter(d=>d&&d.token);let sent=0,failed=0;
  for(const d of ds){try{d.platform==='ios'?await sendIos(d.token,message):await sendAndroid(d.token,message);sent++;}catch(_){failed++;}}
  return{sent,failed};
}
module.exports={sendPushToAccountHash};
