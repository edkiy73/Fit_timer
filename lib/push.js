...
function apnsConfig(){
  ...
  const bundleId=process.env.APNS_BUNDLE_ID||productIdentity().id;
...
async function sendAndroid(token,message){
  ...
  const res=await fetch('https://fcm.googleapis.com/v1/projects/'+encodeURIComponent(a.projectId)+'/messages:send',{method:'POST',headers:{Authorization:'Bearer '+a.token,'Content-Type':'application/json'},body:JSON.stringify({message:{token,notification:{title:String(message.title||productIdentity().name),body:String(message.body||'')},data:stringData(message.data),android:{priority:'high',notification:{sound:'default'}}}})});
  if(!res.ok){
...
function sendIos(token,message){return new Promise((resolve,reject)=>{
  ...
  req.end(JSON.stringify({aps:{alert:{title:String(message.title||productIdentity().name),body:String(message.body||'')},sound:'default'},fit:stringData(message.data)}));
});}
...
function pushInfo(){
  return {
    android:!!firebaseAccount(),
    ios:!!apnsConfig(),
    firebaseVars:Object.keys(process.env).filter(k=>/^FIREBASE_/.test(k)).sort(),
    apnsVars:Object.keys(process.env).filter(k=>/^APNS_/.test(k)).sort()
  };
}
module.exports={sendPushToAccountHash,notificationPrefs,pushInfo};
