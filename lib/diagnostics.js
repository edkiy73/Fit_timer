...
function redactStack(v,n){
  return String(v==null?'':v)
    .replace(/[\r\t]+/g,' ')
    .replace(/[^\s@]{1,64}@[^\s@.]+(?:\.[^\s@.]+)+/gi,'[email]')
    // Домен/query не нужны, а basename + line/column нужны для поиска в bundle.
    .replace(/https?:\/\/[^\s/]+\/([^\s?#)\]}]+)(?:\?[^\s)\]}]*)?/gi,(m,path)=>{
      const tail=String(path||'').split('/').slice(-2).join('/');
      return tail ? '[app]/'+tail : '[url]';
    })
    .replace(/(["'“”‘’])[^\n]{1,160}\1/g,'[value]')
    .replace(/\s+/g,' ')
    .trim()
...
async function recordClientError(input){
  ...
  const stack=redactStack(input&&input.stack,1400);
  const build=redact(input&&input.build,80);
  ...
  if(!rec){
    rec={sig,kind,name,message,stack,build,first:now,last:now,count:0,platform:{android:0,ios:0,web:0},locale:{ru:0,en:0}};
...
async function clearClientError(sig){
  sig=String(sig||'').trim().toLowerCase();
  if(!/^[a-f0-9]{20}$/.test(sig)) return false;
  await Promise.all([
    store.del('diag:error:'+sig),
    store.del('diag:indexed:'+sig),
    store.removeFromList('diag:errors',sig)
  ]);
  return true;
}

module.exports={recordClientError,clientErrorStats,clearClientError,redact,redactStack};
