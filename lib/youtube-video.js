...
function evidencePrompt(meta,chunk,locale){
  return [
    ...
    'Write exercise names in '+(String(locale||'').toLowerCase().startsWith('ru')?'Russian':'English')+'.',
...
async function extractTranscriptEvidence(settings,transcript,locale){
  ...
  return {
    ...
    confidence:g.confidence,rounds:g.rounds,roundRestSec:g.roundRestSec,exercises:dedup
  };
}
...
function factsToProtocol(facts,locale){
  ...
  const desc=(ru?'Собрано по подтверждённым данным из видео. Источник: ':'Built from verified details in the video. Source: ')+facts.url;
  const lines=[
    'ПРОГРАММА: '+name,
    'ОПИСАНИЕ ПРОГРАММЫ: '+protocolLineText(desc,1000),
    'ПРОГРЕССИЯ: нет',
    'ЧЕРЕДОВАНИЕ: нет',
    '',
    'ДЕНЬ:',
