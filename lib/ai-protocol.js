...
- Do not change unrelated fields.`;
  ...
  function validateExerciseResponse(raw){
    ...
    return {ok: !missing.length && count === 1, text, missing,
      reason: missing.length ? 'missing_fields' : (count !== 1 ? 'exercise_count' : '')};
  }
  ...
  function validateResponse(kind, raw){
    ...
    if(/^exercise\.(?:create|modify|replace)$/.test(String(kind || ''))) return validateExerciseResponse(raw);
