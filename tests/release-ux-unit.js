...
need(account.includes("FitTimerModules.runtimeCompat.installUpdate(APP_UPDATE.url,APP_UPDATE.latest)"), 'direct channel must download/install inside Fit Timer');
need(account.includes("FitTimerModules.runtimeCompat.openExternal(APP_UPDATE.url)"), 'store channel must keep external store routing');
need(account.indexOf("APP_UPDATE.channel==='direct'") < account.indexOf("FitTimerModules.runtimeCompat.openExternal(APP_UPDATE.url)"), 'direct branch must run before external store routing');
