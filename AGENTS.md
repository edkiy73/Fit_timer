...
- Reusable runtime Core: `src/core/*.ts`. `src/core/*.runtime.js` is generated compatibility output for the current concatenated frontend; rebuild/check it with `npm run build:core` / `npm run check:core`, never edit it directly.
...
- Generated compatibility files: `index.html`, `style.css`, `app.js`, `appbase-core.js`, and `src/core/*.runtime.js`; rebuild with the matching build scripts and do not edit them directly.
