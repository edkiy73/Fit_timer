...
- use ES modules for Core imports/exports; the current browser bridge uses generated `core/*.js` modules plus dynamic `import()` from legacy code. Add a bundler when it solves a concrete dependency/build problem rather than by default;
...
- Reusable runtime Core: canonical TypeScript lives in `src/core/*.ts`; browser ES modules are generated under `core/*.js` with `npm run build:core` and verified by `npm run check:core`. Do not edit generated `core/*.js` directly.
...
- Generated compatibility files: `index.html`, `style.css`, `app.js`, and `core/*.js`; rebuild with the matching build scripts and do not edit them directly.
