...
- Legacy product dependency bridge: only `src/app/00-dependencies.js` may read temporary `globalThis.FitTimerModules`; all other product files use the captured `app*` bindings. `src/main.ts` deletes the bridge immediately after `app.js` loads.
- Generated compatibility files: `index.html`, `style.css`, and `app.js`; rebuild with the matching build scripts and do not edit them directly.
