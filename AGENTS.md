...
Do **not** read all of `CLAUDE.md`, `style.css`, or `index.html` by default.
...
- Reusable runtime Core: `src/core/*.ts`, loaded through the production ESM entry `src/main.ts`. Browser Core runtime mirrors are no longer generated.
...
- Product runtime: ES modules `src/app/NN-*.js`, entry `src/app/index.js` (lazily imported by `src/main.ts`). A part's top level only declares; listeners/handlers/timers go into its exported `init*()`, which the entry calls in part order. Import every name you use from its owner module; constants read at load time belong in leaf modules such as `src/app/options.js`. `tests/app-module-graph-unit.js` guards this.
- Generated compatibility files: `index.html` and `style.css`; rebuild with the matching build scripts and do not edit them directly.
- Mobile product adapter: `mobile.js`, loaded by the ESM entry after reusable mobile/native Core modules are ready.
...
- Trainer -> `api/trainer/[handle].js`, trainer code in `src/app/50-trainer-catalog.js`, targeted trainer tests.
- Catalog -> `api/catalog.js`, `admin.html`, catalog code in `src/app/50-trainer-catalog.js`.
- Sync -> `api/sync.js`, sync symbols in `src/app/10-data-sync.js`, `tests/sync-*.js`.
...
- Do not edit generated `dist/`, root `index.html`, or root `style.css`; edit `src/**` and rebuild.
...
- For `style.css` / `index.html`, fetch only the surrounding range of matched symbols/selectors.
